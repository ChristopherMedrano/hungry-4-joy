...
export function EmptyState({ onResetFilters }: EmptyStateProps) {
  return (
    <div className="rounded-lg border border-dashed border-slate-700 bg-slate-900/30 px-6 py-16 text-center">
      <p className="text-base font-medium text-slate-200">No checkout events match</p>
      <p className="mt-2 text-sm text-slate-400">
        Adjust filters or wait for new webhook events once the Laravel dashboard API is
        connected.
      </p>
      {onResetFilters ? (
        <button
