...
Donation buttons keep static campaign and option metadata in markup. A small theme script generates an opaque `donation_attempt_id` at click time and appends it to the Foxy cart URL before handoff.
...
After a click, the outgoing Foxy cart URL should include an added `donation_attempt_id` option. Do not render this value as static markup; each click should create a fresh attempt identity.
...
This metadata is inspectable in browser developer tools. The static Foxy demo cart link should contain matching `name`, `price`, `code`, `quantity`, `donation_label`, `donation_type`, `source_page`, `campaign_name`, and `checkout_provider` parameters. After the click handler runs, the outgoing URL should also include `donation_attempt_id`.
...
| Click-time generated value | Foxy `donation_attempt_id` option. |
...
- Opaque per-click `donation_attempt_id` generation.
...
- Canonical `donation_attempt_id` validation and storage.
...
- Clicking a donation option appends an opaque `donation_attempt_id` to the cart URL.
